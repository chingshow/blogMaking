...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const isIntroPage = url.pathname.includes('intro.html');
    ...
    fetch('content.json')
        ...
        .then(data => {
            ...
            if (currentFilterType && currentFilterValue) {
                filterArticles(currentFilterType, currentFilterValue);
            } else if (isIntroPage) {
                displayIntroPage();
                populateSidebar();
            } else if (tag) {
...
function displayIntroPage() {
    isIntroPage = true;
    const introContainer = document.getElementById('main');
...
function filterArticles(type, value) {
    // 如果当前在Intro.html页面,则更新筛选条件并跳转到index.html
    if (isIntroPage) {
